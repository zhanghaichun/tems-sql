...
BEGIN
  ...
  /**
   * 此 SQL 程序的主要逻辑是：
   * 1. 将 rate_rule_contract_original 表中新插入的数据同步到 rate module 相关的表中。
   */
  ...
  IF ( V_NOT_UPDATE_RECORD_COUNT > 0 ) THEN
    ...
    IF(V_AUDIT_REFERENCE_MAPPING_ITEM_COUNT = 0) THEN
      ...
      /**
       * 1. 向 contract_file 表中插入相关 field value.
       */
      ...
      /**
       * 2. 向 contract 表中插入相关 field value.
       */
    ...
    ELSE
      ...
      /**
       * 即使是插入了在 mapping 表中能够匹配到 key_field 的数据，
       * 也需要到 contract_file 表中查询是否有对应 contract_number 
       * 和 effective date 的记录， 如果没有，同样需要插入一条记录，
       * 并更新 contract 表中对应记录的 contract_file_id.
       */
      /**
       * 1. 向 contract_file 表中插入相关 field value.
       */
      ...
      /**
       * 更新 contract_file_id 和 schedule.
       */
      UPDATE contract
      SET 
    ...
    ELSEIF(V_REFERENCE_TABLE = 'contract_rate_by_quantity') THEN
      ...
      /**
       * 判断 contract_rate_by_quantity 表中是否有当前 volume 记录
       */
      ...
      IF(V_CONTRACT_RATE_BY_QUANTITY_ITEM_COUNT = 0) THEN
        /**
         * contract_rate_by_quantity 表中没有当前记录。
         */
      ...
      ELSE
        /**
         * contract_rate_by_quantity 表中有当前记录
         */
