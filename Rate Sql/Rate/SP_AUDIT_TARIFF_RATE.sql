...
BEGIN
    ...
    --  这两个字段仅仅被用来向 audit_result 表插入。 
    SELECT invoice_id, product_id 
        INTO v_invoice_id,v_product_id
    FROM proposal
    WHERE id = val_proposal_id;

    -- 1.Rate Mode: 不同的 rate mode 可能获取 rate 的方式是不同的
    -- 2.Discount: rate * quantity 算出来之后还需要与 discount 相乘， rate mode 中肯定包含 discount
    -- 3.Multiplier: rate * quantity 算出来之后还需要与 multiplier 相乘， rate mode 中肯定包含 multiplier
    -- 4.Tolerance rate: 有些明细的 actual amount 和 expect amount 虽然是不相等的，但是如果这条 rate rule 
    --  中包含 tolerance rate, 那么需要查看是否 actual amount 和 expect amount 的差值是在 tolerance rate
    --  的范围内，如果在这个范围之内，那么验证也是通过的
    ...
    -- 获取 Rate 和 Rate Effective Date, 但是只适用于部分 Rate Mode, 对于 quantity
    -- 等具有特殊逻辑的 tariff, 会有其他的逻辑进行处理。
    ...
    -- 1. Payment amount : actual amount 账单上的实际金额
    -- 2. quantity: 账单明细中的数量
    -- 3. province_id
